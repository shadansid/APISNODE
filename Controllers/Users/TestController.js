 ...
 let fee = (feepercentage*inputAmountX)/100;
  ...
  if (!custom) {
    const Cp = await axios.get(
      `https://api.binance.com/api/v3/ticker/price?symbol=${inputSymbol}`
    );
    ...
    connection.query(
      ...
      (err, result) => {
        ...
        if (result.length) {
          ...
          if (result[0].quantity > inputAmountX) {
            let jnkn = inputAmountX;
                ...
                connection.query(
                  ...
                  (err, result) => {
                    ...
                    if (result.length) {
                        ...
                        (err, result) => {
                          ...
                          connection.query(upsql, (err, result) => {
                            ...
                            let updatedCustomQuan = result[0].quantity - CoinQuantity
                              let updatedCustomPrice =  result[0].price + 0.0001
                            connection.query(
                      ...
                      let TH = `INSERT INTO transactionhistory (UserId , quantity , amount,quantityvalue, currency , type,orderid,status,fee,feepercentage,total) VALUES("${req.token.userid}", "${CoinQuantity}" , "${CoinPrice}" ,"${CoinQuantity*CoinPrice}", "${Coin}" ,"credit","${orderid}","success","${fee}","${feepercentage}","${inputAmount+fee}" )`;
              ...
              connection.query(
                ...
                (err, result) => {
                  ...
                  if (result.length) {
                    ...
                    connection.query(
                      sql,
                      [{ quantity: finalCoinQuantity }, req.token.userid, Coin],
                      (err, result) => {
                        if (err) throw err;
                        connection.query(`COMMIT`);
                      }
