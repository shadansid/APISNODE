      ...
      (err, result) => {
        ...
        if (result.length) {
          ...
          if (result[0].quantity * backedCoinPrice > inputAmount) {
            ...
            connection.query(
              sql,
              [{ quantity: newQuantity }, req.token.userid, Coin],
              (err, result) => {
                if (err) throw err;
                  ...
                  (err, result) => {
                    ...
                    if (result.length) {
                      ...
                      connection.query(
                        sql,
                        [
                          { quantity: finalbackedCoinQuantity },
                          req.token.userid,
                          backedCoin,
                        ],
                        (err, result) => {
                          ...
                          connection.query(upsql, (err, result) => {
                            ...
                            let updatedCustomQuan = result[0].quantity - backedCoinQuantity
                    ...
                    if(result.length){
                            ...
                            console.log('inside if')
                                ...
                                connection.query(TH,(err, results)=>{
                                    if(err) throw err;
                                  console.log("histry add hogyi")
                                  return res.json({ msg: "Trade Success" });
