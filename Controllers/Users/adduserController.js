...
connection.query("SELECT * FROM users WHERE email = ? ", [req.body.email], function(error, results, fields) {
    ...
    if(results.length === 0){
      ...
      var sql = `INSERT INTO users ( userid,name, email, password, Active, Role, verified) VALUES ("${userId}","${req.body.name}", "${req.body.email}","${password}","1","0","1")`;
      ...
      connection.query(sql, function (err, result) {
        ...
        req.yourcode = Yourcode.toUpperCase()
